...
RandomAnglesModel5FULL = np.genfromtxt("./CCREG/R5SamplesSorted.csv", delimiter=',', skip_header=1)
...
for _ in range(N_DOWNSAMPLING):
    ...
    for rowIndex in range(0, len(RandomAnglesModel5FULL)):
        print(f"Processing row {rowIndex}...")
        RandomAnglesModel5 = RandomAnglesModel5FULL[rowIndex]
        # Downsample the data     
        ...
        for angles, labels, output_prefix in [
            (CurrentFixationAngles, CurrentFixationAnglesClass, "Random5"),
        ]:
            #print(f"PROCESSING {output_prefix}...")

            #Null case for this model
            for train_index, test_index in kf.split(RandomAnglesModel5, labels):
                #print("     FOLD...")
                X_train, X_test = RandomAnglesModel5[train_index], RandomAnglesModel5[test_index]
